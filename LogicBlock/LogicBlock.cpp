 ...
 *   https://www.teledynevisionsolutions.com/support/support-center/application-note/iis/using-logic-blocks/
 *
...
int ConfigureLogicBlock(INodeMap& nodeMap)
{
    ...
    if (!IsReadable(ptrLogicBlockSelector) || !IsWritable(ptrLogicBlockSelector))
    {
        cout << "Unable to get or set logic block selector to Logic Block 0 (node retrieval). Non-fatal error..."
             << endl;
        return -1;
    }
    ...
    if (!IsReadable(ptrLBLUTSelector) || !IsWritable(ptrLBLUTSelector))
    {
    ...
    if (!IsReadable(ptrLBLUTInputSelector) || !IsWritable(ptrLBLUTInputSelector))
    {
        cout << "Unable to get or set LUT logic block input selector to Input 0 (node retrieval). Non-fatal error..."
             << endl;
        return -1;
    }
    ...
    if (!IsReadable(ptrLBLUTSource) || !IsWritable(ptrLBLUTSource))
    {
        cout << "Unable to get or set LUT logic block input source to Frame Trigger Wait (node retrieval). Non-fatal "
                "error..."
             << endl;
        return -1;
    }
    ...
    if (!IsReadable(ptrLBLUTActivation) || !IsWritable(ptrLBLUTActivation))
    {
        cout << "Unable to get or set LUT logic block input activation to level high (node retrieval). Non-fatal "
                "error..."
             << endl;
        return -1;
    }
...
int GrabTwoImages(INodeMap& nodeMap, CameraPtr pCam)
{
    ...
    try
    {
        ...
        if (!IsReadable(ptrUserOutputSelector) || !IsWritable(ptrUserOutputSelector))
        {
