...
int RunSingleCamera(CameraPtr pCam)
{
    ...
    try
    {
        ...
        // This example is not compatible with BX (Bumblebee) stereo cameras, see StereoAcquisiton for this camera.
        CStringPtr ptrDeviceModelName = nodeMapTLDevice.GetNode("DeviceModelName");
        if (IsReadable(ptrDeviceModelName))
        {
            string deviceModelName = ptrDeviceModelName->GetValue().c_str();
            if (deviceModelName.find("BX") != std::string::npos)
            {
                cout << "This example is not compatible with BX (Bumblebee) stereo cameras. Please see StereoAcquisition Example for ChunkData usage with this camera." << endl;
                return -1;
            }
        }
