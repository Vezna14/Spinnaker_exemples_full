...
void AutoConfigure()
{
    cout << "---> Setting all GigE cameras discovered to an IP configuration that will work with Spinnaker:" << endl
         << endl;
    ...
    for (unsigned int i = 0; i < interfaceCount; i++)
    {
        ...
        //
        CStringPtr ptrInterfaceDisplayName = nodeMapInterface.GetNode("InterfaceDisplayName");
        gcstring interfaceDisplayName = "Unknown Interface (Display name not readable)";
        if (IsAvailable(ptrInterfaceDisplayName) && IsReadable(ptrInterfaceDisplayName))
        {
        ...
        if (IsAvailable(ptrAutoForceIP) && IsWritable(ptrAutoForceIP))
        {
            ...
            {
                const unsigned int cameraCountPreForceIP = pInterface->GetCameras().GetSize();
                
                cout << "Executing AutoForceIP on " << cameraCountPreForceIP << " camera(s) on " << interfaceDisplayName << endl;
                
                // Set each device selector from largest index down to zero so cameras that re-enumerate do not affect
                // the order of the device list
                for (unsigned int index = 0; index < cameraCountPreForceIP; index++)
                {
                    int deviceSelectorIndex = cameraCountPreForceIP-index-1;
                    pInterface->TLInterface.DeviceSelector.SetValue(deviceSelectorIndex);
                    pInterface->TLInterface.GevDeviceAutoForceIP.Execute();
                    cout << "AutoForceIP executed for camera at index " << deviceSelectorIndex << ", serial number: " << pInterface->TLInterface.DeviceSerialNumber.GetValue() << endl;
                }
            }
        }
        else 
        {
            cout << "Force IP node not available for this interface (e.g. No Gige Cameras are connected to this interface)." << endl;
        }
        cout << endl;
    }
