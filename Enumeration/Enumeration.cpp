...
int QueryInterface(InterfacePtr pInterface)
{
    ...
    try
    {
        ...
        //
        // Print interface's producer name
        //
        // *** NOTES ***
        // Each interface is enumerated from a producer that can be retrieved in order to
        // access information about the producer itself, or any devices connected.
        //
        ProducerPtr pProducerIface = pInterface->GetProducer();
        cout << "\tProducer: " << pProducerIface->TLSystem.TLDisplayName.GetValue() << endl;
        pProducerIface = nullptr;
        ...
        for (unsigned int i = 0; i < numCameras; i++)
        {
            ...
            //
            // Print camera's producer name
            //
            // *** NOTES ***
            // Each camera is enumerated from a producer that can be retrieved in order to
            // access information about the producer itself, or any devices connected.
            //
            ProducerPtr pProducerCam = pCam->GetProducer();
            cout << "\tProducer: " << pProducerCam->TLSystem.TLDisplayName.GetValue() << endl;
            pProducerCam = nullptr;
        }
...
// This function queries a producer for its interfaces and cameras and then prints out producer
// information.
int QueryProducer(ProducerPtr pProducer)
{
    int result = 0;

    try
    {
        cout << "Producer Name: " << pProducer->TLSystem.TLDisplayName.GetValue() << endl;
        cout << "\tFileName: " << pProducer->TLSystem.TLFileName.GetValue() << endl;
        cout << "\tID: " << pProducer->TLSystem.TLID.GetValue() << endl;
        cout << "\tPath: " << pProducer->TLSystem.TLPath.GetValue() << endl;
        // cout << "\tType: " << producer->TLSystem.TLType.GetValue() << endl;

        cout << " \tFilePath: " << pProducer->GetProducerFilePath() << endl << endl;

        //
        // Retrieve list of interfaces from the producer
        //
        // *** NOTES ***
        // Interface lists are retrieved from a producer or system object.
        // See below for an example of retrieving from the system object.
        //
        // *** LATER ***
        ...
        //
        InterfaceList interfaceList = pProducer->GetInterfaces();
        unsigned int numInterfaces = interfaceList.GetSize();

        cout << "\tNumber of interfaces detected: " << numInterfaces << endl;

        //
        // Retrieve list of cameras from the producer
        //
        // *** NOTES ***
        // Camera lists can be retrieved from an interface, producer, or the system object.
        // Camera lists retrieved from a producer, such as this one, return all
        // cameras available on the producer.
        // See below for an example of retrieving from the system object.
        //
        // *** LATER ***
        // Camera lists must be cleared manually. This must be done prior to
        // releasing the system and while the camera list is still in scope.
        //
        CameraList camList = pProducer->GetCameras();
        unsigned int numCameras = camList.GetSize();

        cout << "\tNumber of cameras detected: " << numCameras << endl << endl;

        // Clear interface and camera lists before losing scope
        interfaceList.Clear();
        camList.Clear();
    }
    catch (Spinnaker::Exception& e)
...
int main(int /*argc*/, char** /*argv*/)
{
    ...
    cout << "Number of GenTL producers detected: " << numProducers << endl << endl;
    ...
    cout << endl << "*** QUERYING PRODUCERS ***" << endl << endl;

    //
    // Create shared pointer producer
    //
    // *** NOTES ***
    // The ProducerPtr object is a smart pointer, and will generally clean
    // itself up upon exiting its scope.
    //
    // *** LATER ***
    // However, if a smart producer pointer is created in the same scope that
    // a system object is explicitly released (i.e. this scope), the reference to
    // the producer must be broken by manually setting the pointer to nullptr.
    //
    ProducerPtr producerPtr = nullptr;

    for (unsigned int i = 0; i < numProducers; i++)
    {
        // Select producer
        producerPtr = producerList.GetByIndex(i);

        // Query interface
        result = result | QueryProducer(producerPtr);
    }

    //
    // Release reference to the producer
    //
    // *** NOTES ***
    // Had the ProducerPtr object been created within the for-loop, it would
    ...
    //
    // *** NOTES ***
    // See notes for smart producer pointer above.
    //
    ...
    //
    // *** NOTES ***
    // Producer lists must be cleared manually prior to a system release call.
    //
    producerList.Clear();
