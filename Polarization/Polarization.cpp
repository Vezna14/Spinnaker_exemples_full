 ...
 *  https://www.teledynevisionsolutions.com/learn/learning-center/machine-vision/imaging-reflective-surfaces-sonys-first-polarized-sensor/
 *
...
int CreateHeatmapImages(const ImagePtr& mono8Image, const string baseFilename, gcstring& deviceSerialNumber)
{
    try
    {
        ...
        //
        ImageUtilityHeatmap::SetHeatmapColorGradient(SPINNAKER_HEATMAP_COLOR_BLACK, SPINNAKER_HEATMAP_COLOR_WHITE);
...
int ExtractAndSavePolarQuadImages(const ImagePtr& pRawPolarizedImage, gcstring& deviceSerialNumber)
{
    try
    {
        ...
        array<PolarizationQuadrant, 4> polarizationQuadEnums = {SPINNAKER_POLARIZATION_QUADRANT_I0,
        ...
        for (auto stokesImage = stokesImages.begin(); stokesImage != stokesImages.end(); ++stokesImage)
        {
            ...
            CreateNormalizedImage(
                *stokesImage,
                stokesName,
                deviceSerialNumber,
                SourceDataRange::SPINNAKER_SOURCE_DATA_RANGE_ABSOLUTE_DATA_RANGE);
        }
    }
    catch (Spinnaker::Exception& e)
    {
