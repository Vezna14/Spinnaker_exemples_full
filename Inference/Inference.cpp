...
bool CameraDeleteFile(INodeMap& nodeMap)
{
    ...
    try
    {
        ...
        if (!IsReadable(ptrFileOperationSelector) || !IsWritable(ptrFileOperationSelector))
        {
...
bool CameraOpenFile(INodeMap& nodeMap)
{
    ...
    try
    {
        ...
        if (!IsReadable(ptrFileOpenMode) || !IsWritable(ptrFileOpenMode))
        {
...
int UploadFileToCamera(INodeMap& nodeMap, const std::string& fileSelectorEntryName, const std::string& filePath)
{
    ...
    if (!IsReadable(ptrFileSelector) || !IsWritable(ptrFileSelector))
    {
...
int ConfigureInference(INodeMap& nodeMap, bool isEnabled)
{
    ...
    try
    {
        if (isEnabled)
        {
            ...
            if (!IsReadable(ptrInferenceNetworkTypeSelector) || !IsWritable(ptrInferenceNetworkTypeSelector))
            {
...
int ConfigureTestPattern(INodeMap& nodeMap, bool isEnabled)
{
    ...
    try
    {
        ...
        if (!IsReadable(ptrTestPatternGeneratorSelector) || !IsWritable(ptrTestPatternGeneratorSelector))
        {
        ...
        if (isEnabled)
        {
            ...
            if (!IsReadable(ptrInjectedWidth) || !IsWritable(ptrInjectedWidth))
            {
            ...
            if (!IsReadable(ptrInjectedHeight) || !IsWritable(ptrInjectedHeight))
            {
